...
class Component {
  ...
  draw(ctx) {
    ...
    if (this.type === "battery") {
      ...
      bodyGrad.addColorStop(0, "#f39c12"); // Darker edge
      bodyGrad.addColorStop(0.4, "#f1c40f"); // Highligh
      bodyGrad.addColorStop(1, "#d35400"); // Shadow edge
      ctx.fillStyle = bodyGrad;
      ctx.fillRect(-(width / 2), -(height / 2), width, height);

      // Terminals Caps Gradient (Metallic)
      const capGrad = ctx.createLinearGradient(0, -height / 2, 0, height / 2);
      capGrad.addColorStop(0, "#7f8c8d");
      capGrad.addColorStop(0.4, "#bdc3c7");
      capGrad.addColorStop(1, "#7f8c8d");

      ctx.fillStyle = capGrad; // Silver/Grey
      // Positive Cap (Left)
      ...
      ctx.fillStyle = "rgba(0, 0, 0, 0.7)"; // Darker Black -
      ctx.fillText("-", width / 4, -1);
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(-7, 5); // From base left
      ctx.lineTo(-5, -12); // To filament left
      ctx.moveTo(7, 5); // From base right
      ctx.lineTo(5, -12); // To filament right
      ctx.stroke();

      // Filament Coil (Zigzag)
      ctx.beginPath();
      ctx.moveTo(-5, -12);
      // Draw a small coil
      const coilW = 10;
      ...
      // Filament Color
      if (brightness > 0.01) {
          const intensity = Math.min(brightness, 1.2) / 1.2;
          // Match Glass Hue Logic
          const hue = 35 + intensity * 25; 
          const light = 40 + intensity * 60; // 40 -> 100
          ...
          ctx.strokeStyle = "#555"; // Dark unlit filament
          ctx.lineWidth = 1.5;
      }
      ...
      // Bulb Glass Gradient (Radial for spherical look)
      const glassGrad = ctx.createRadialGradient(-5, -20, 2, 0, -15, 22);
      if (brightness > 0.01) {
        // Lit State - Dynamic intensity
        // Nonlinear intensity for better visual range
        // brightness is usually V / 1.5. So 1.5V = 1.0. 0.75V = 0.5.
        const intensity = Math.min(brightness, 1.2) / 1.2; 
        
        // Deep Yellow (Amber) <-> Bright Yellow
        // Hue: 35 (Deep Amber) -> 60 (Yellow)
        const hue = 35 + intensity * 25; 
        
        // Lightness: 30% (Dark) -> 90% (Bright)
        const coreL = 30 + intensity * 60; 
        
        // Alpha: 0.5 (Semi-transparent) -> 1.0 (Solid glow)
        const alpha = 0.5 + intensity * 0.5;
        
        // Highlight (Center)
        glassGrad.addColorStop(0, `hsla(${hue}, 100%, ${Math.min(coreL + 40, 100)}%, ${alpha})`); 
        // Core
        glassGrad.addColorStop(0.4, `hsla(${hue}, 100%, ${coreL}%, ${alpha})`); 
        // Edge (Darker)
        glassGrad.addColorStop(1, `hsla(${hue}, 100%, ${coreL * 0.6}%, ${alpha * 0.8})`); 
      } else {
        // Unlit State
        glassGrad.addColorStop(0, "rgba(255, 255, 255, 0.4)"); // Reflection
        glassGrad.addColorStop(0.3, "rgba(236, 240, 241, 0.3)"); // Glass body
        glassGrad.addColorStop(1, "rgba(189, 195, 199, 0.2)"); // Edge
      }
      ...
      // Screw base (Thread) Gradient
      const baseGrad = ctx.createLinearGradient(-12, 0, 12, 0); // Horizontal cylinder
      baseGrad.addColorStop(0, "#7f8c8d");
      baseGrad.addColorStop(0.5, "#ecf0f1"); // Shiny middle
      baseGrad.addColorStop(1, "#7f8c8d");
      ctx.fillStyle = baseGrad; // Metal
      ...
      // 1. Motor Body (Cylinderish) - Metallic Gradient (High Contrast)
      const motorBodyGrad = ctx.createLinearGradient(-25, 0, 25, 0);
      motorBodyGrad.addColorStop(0, "#34495e"); // Dark Edge
      motorBodyGrad.addColorStop(0.2, "#7f8c8d");
      motorBodyGrad.addColorStop(0.4, "#ecf0f1"); // Bright Highlight
      motorBodyGrad.addColorStop(0.6, "#95a5a6");
      motorBodyGrad.addColorStop(1, "#34495e"); // Dark Edge
      
      ctx.fillStyle = motorBodyGrad;
      ...
      // Apply the same metallic gradient to this face so it matches
      ctx.fillStyle = motorBodyGrad; 
      ctx.fill();
      ...
      shaftGrad.addColorStop(0, "#2c3e50");
      shaftGrad.addColorStop(0.5, "#95a5a6");
      ...
      // Draw 3 Blades (Propeller Shape)
      const bladeGrad = ctx.createLinearGradient(0, 0, 0, -45);
      bladeGrad.addColorStop(0, "#3498db"); // Blue plastic hub
      bladeGrad.addColorStop(1, "rgba(52, 152, 219, 0.6)"); // Transparent tip
      
      ctx.fillStyle = bladeGrad; 
      for (let i = 0; i < 3; i++) {
        ctx.beginPath();
        // Propeller shape: Narrow at base, wider at tip, curved trailing edge
        ctx.moveTo(0, 0);
        ctx.bezierCurveTo(15, -5, 25, -30, 10, -45); // Leading Edge (Curved out)
        ctx.bezierCurveTo(-5, -45, -15, -20, 0, 0); // Trailing Edge (Straighter return)
      ...
      // Base (Insulator) - Plastic Gradient
      const baseGrad = ctx.createLinearGradient(-40, -10, -40, 30);
      baseGrad.addColorStop(0, "#ecf0f1");
      baseGrad.addColorStop(1, "#bdc3c7");
      ...
      // Blade (Copper) - Metallic Gradient
      const bladeGrad = ctx.createLinearGradient(0, -4, 0, 4);
      bladeGrad.addColorStop(0, "#f39c12");
      bladeGrad.addColorStop(0.5, "#f1c40f");
      bladeGrad.addColorStop(1, "#d35400");
      ctx.fillStyle = bladeGrad;
      ...
      // Handle (Insulator) - Red Plastic Gradient
      const handleGrad = ctx.createRadialGradient(60, 0, 2, 60, 0, 6);
      handleGrad.addColorStop(0, "#e74c3c");
      handleGrad.addColorStop(1, "#c0392b");
      ...
      // Proper Gem Paperclip Shape
      ctx.lineJoin = "round";
      ctx.lineCap = "round";
      
      // Metallic Gradient Stroke
      const clipGrad = ctx.createLinearGradient(0, -15, 0, 15);
      clipGrad.addColorStop(0, "#bdc3c7");
      clipGrad.addColorStop(0.4, "#ecf0f1"); // Highlight
      clipGrad.addColorStop(0.6, "#95a5a6");
      clipGrad.addColorStop(1, "#7f8c8d");
      
      ctx.strokeStyle = clipGrad;
      ctx.lineWidth = 4;

      ctx.beginPath();
      // Draw standard double-loop gem clip
      // Start further left to make inner loop longer
      ctx.moveTo(-10, 0); 
      // 1. Go Right to inner-right turn
      ctx.lineTo(25, 0);
      // 2. Inner Right Turn (Small) - Upwards
      ctx.arc(25, -5, 5, Math.PI/2, -Math.PI/2, true); // Center (25, -5), R=5, CounterCW
      // 3. Go Left (Inner Top Straight)
      ctx.lineTo(-25, -10);
      // 4. Left Turn (Medium) - Downwards
      ctx.arc(-25, 0, 10, -Math.PI/2, Math.PI/2, true); // Center (-25, 0), R=10
      // 5. Go Right (Outer Bottom Straight)
      ctx.lineTo(25, 10);
      // 6. Right Turn (Large) - Upwards
      ctx.arc(25, -2, 12, Math.PI/2, -Math.PI/2, true); // Center (25, -2), R=12 (wider)
      // 7. Go Left (Outer Top Straight) - End of clip stroke
      ctx.lineTo(-35, -14);
      ...
      // Rectangular Eraser
      // Body (White/Rubber)
      const rubberGrad = ctx.createLinearGradient(-40, 0, 40, 0);
      rubberGrad.addColorStop(0, "#ecf0f1");
      rubberGrad.addColorStop(0.5, "#ffffff");
      rubberGrad.addColorStop(1, "#bdc3c7");
      ...
      // Sleeve (Blue cardboard) - WIDENED
      const sleeveGrad = ctx.createLinearGradient(0, -15, 0, 15);
      sleeveGrad.addColorStop(0, "#3498db");
      sleeveGrad.addColorStop(0.5, "#5dade2");
      sleeveGrad.addColorStop(1, "#2980b9");
      ctx.fillStyle = sleeveGrad;
      // Widen from 30 (-15 to 15) to 50 (-25 to 25) to fit text
      ...
      // 2x4 Brick
      // Red Brick Gradient
      const legoGrad = ctx.createLinearGradient(0, -15, 0, 15);
      legoGrad.addColorStop(0, "#e74c3c");
      legoGrad.addColorStop(1, "#c0392b");
      ctx.fillStyle = legoGrad;
...
class ChallengeManager {
  ...
  checkAnswer(components, wires) {
    ...
    if (isCorrect) {
      ...
      showModal(
        "挑戰結果",
        `❌ 答錯了！<br><span style="color:#e74c3c; font-size:0.9em">${failReason}</span>`,
        "info"
      ).then(() => {
        this.nextQuestion();
      });
...
function setupGameUI() {
  ...
  // Challenge Setup Modal Elements
  ...
  document.getElementById("clear-btn").addEventListener("click", () => {
    showModal("清除確認", "確定要清除所有元件嗎？", "confirm").then(
      (confirmed) => {
