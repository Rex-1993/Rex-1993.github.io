...
const DRAG = 0.9; // Linear damping
const ANGULAR_DRAG = 0.8; // Angular damping
const MAX_SPEED = 15;
  ...
  getPoles() {
    ...
    if (this.type === "bar_magnet") {
      ...
      return [
        {
          type: "N",
          pos: Vec2.add({ x: this.x, y: this.y }, nPos),
          strength: 1,
        },
        {
          type: "S",
          pos: Vec2.add({ x: this.x, y: this.y }, sPos),
          strength: 1,
        },
      ];
    }
...
function updatePhysics() {
  ...
  magnets.forEach((magnet) => {
    ...
    magneticObjects.forEach((obj) => {
      ...
      if (obj.stuckTo === magnet) return; // If stuck, disable force to stop jitter
      ...
      if (obj.type === "bar_magnet") {
        ...
        mPoles.forEach((p1) => {
          objPoles.forEach((p2) => {
            ...
            if (!magnet.isStatic) {
              ...
              if (reactionScale > 0) {
                magnet.applyForce(
                  Vec2.scale(force, -1 * reactionScale),
                  p1.pos
                );
              }
            }
        ...
        const end1 = Vec2.add(
          { x: obj.x, y: obj.y },
          Vec2.rotate({ x: -len, y: 0 }, obj.rotation)
        );
        const end2 = Vec2.add(
          { x: obj.x, y: obj.y },
          Vec2.rotate({ x: len, y: 0 }, obj.rotation)
  ...
  components.forEach((c) => {
    ...
    if (currentSpeed < 2.0) {
      c.vx *= 0.8; // Stronger drag (Static friction feel)
      c.vy *= 0.8;
      c.angV *= 0.8;
    } else {
      c.vx *= DRAG;
  ...
  // Reset stuckTo state BEFORE re-evaluating collisions
  components.forEach(c => c.stuckTo = null); 
...
function resolveCollision(c1, c2) {
  ...
  if (result) {
    ...
    // If one is static (dragged), move the other fully.
    const totalMass =
      (c1.isStatic ? 10000 : c1.mass) + (c2.isStatic ? 10000 : c2.mass);
    ...
    if (c1.mass / c2.mass > 10 && !c1.isStatic) {
      m1Ratio = 0;
      m2Ratio = 1;
    }
    if (c2.mass / c1.mass > 10 && !c2.isStatic) {
      m2Ratio = 0;
      m1Ratio = 1;
    }
    ...
    // Check for Magnetic Locking (Sticking)
    const stickyTypes = ["bar_magnet", "iron_nail", "key"];
    const isMagneticStick =
      (c1.type === "bar_magnet" && stickyTypes.includes(c2.type)) ||
      (c2.type === "bar_magnet" && stickyTypes.includes(c1.type));

    // Soft Positional Correction (Baumgarte Stabilization)
    // If welding (relSpeed < 3.0), DISABLE separation to allow equilibrium
    const approxRelSpeed = Vec2.mag(
      Vec2.sub({ x: c1.vx, y: c1.vy }, { x: c2.vx, y: c2.vy })
    );
    ...
    // Relative velocity
    const rv = Vec2.sub({ x: c1.vx, y: c1.vy }, { x: c2.vx, y: c2.vy });
    ...
    // ALSO: If interaction involves a magnet and a magnetic object, FORCE stick.
    const relSpeed = Vec2.mag(rv);

    if (relSpeed < 3.0 || isMagneticStick) {

      // Flag as stuck to disable magnetic force in next frame
      if (isMagneticStick) {
        if (c1.type === "bar_magnet") c2.stuckTo = c1;
        if (c2.type === "bar_magnet") c1.stuckTo = c2;
      }
...
function drawFieldLines(ctx) {
  ...
  magnets.forEach((m) => {
    ...
    for (let i = -2; i <= 2; i++) {
      ...
      const pos = Vec2.add(
        { x: m.x, y: m.y },
        Vec2.rotate({ x: -w / 2, y: localY }, m.rotation)
      );
...
clearBtn.addEventListener("click", () => {
  showModal("清除確認", "確定要清除所有物品嗎？", "confirm").then(
    (confirmed) => {
      if (confirmed) {
        components = [];
      }
...
if (btnOpenInstructions)
  btnOpenInstructions.addEventListener("click", showInstructions);
if (btnCloseInstructions)
  btnCloseInstructions.addEventListener("click", hideInstructions);
if (btnSidebarInstructions)
  btnSidebarInstructions.addEventListener("click", showInstructions);
